...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h4 class="h4">Department >> Edit Department</h4>
        <div class="btn-toolbar mb-2 mb-md-0">
            <div class="btn-group me-2">
                <a href="new_department.php" class="btn btn-sm btn-outline-secondary">Add Department</a>
            </div>
        </div>
    </div>
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <div class="container">
            <div class="row">
                ...
                <div class = "col-md-7 col-lg-10">
                    ...
                    <?php
                    extract($_POST);
                    if ($_SERVER['REQUEST_METHOD'] == "POST" && @$operate_edit == 'edit') {
                        $db = dbConn();
                        $sql = "SELECT * FROM tbl_department d INNER JOIN tbl_office o ON d.OfficeID = o.OfficeID WHERE DepartmentID='$DepartmentID'";
                        $result = $db->query($sql);
                        $row = $result->fetch_assoc();
//                        print_r($row);
                        $d_DptName = $row['DptName'];
                        $d_Location = $row['Location'];
                        $d_Head = $row['Head'];
                    }
                    // if request is 'POST'and butten is click do
...
//                        Convert array keeys to variable
                        ...
                        if (!empty($d_DptName)) {
                            $db = dbConn();
//                            $sql = "SELECT * FROM tbl_department WHERE DptName='$d_DptName' AND DepartmentID <> '$DepartmentID'";
...
//                        print_r($messages);
                    ...
                    <form method = "post" action = "<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>" class = "card p-2">
                        ...
                        <div class = "row g-3">
                            ...
                            <div>
<!--                                <input type="text" name="DepartmentID" value="<?= $row['DepartmentID'] ?>">-->
                                <button class = "w-50 btn btn-primary btn-lg" type = "submit" name="operate" value="update">Update</button>
                            </div>
                    </form>
                </div>
            </div>
